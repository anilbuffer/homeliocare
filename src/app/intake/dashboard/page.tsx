...
export default function IntakeDashboard() {
  ...
  const toggleAudioBriefing = () => {
    ...
    const text = `${greeting} James. You have 2 urgent hospital discharges pending S L A breach for Eleanor Vance and Arthur Pendelton. 3 assessments are unassigned for tomorrow.`;
        ...
        <div className="w-full bg-gradient-to-r from-brand-teal/10 to-accent-blue/10 border border-brand-teal/20 rounded-2xl p-4 flex flex-col sm:flex-row items-start sm:items-center gap-4 shadow-[0_6px_32px_rgba(0,0,0,0.06)] relative overflow-hidden">
          ...
          <div className="flex-1 z-10">
            ...
            <p className="text-xs text-slate-700 font-medium leading-relaxed">
              {greeting} James. You have <span className="text-red-600 font-semibold bg-red-50 px-1 rounded">2 urgent hospital discharges pending SLA breach</span> (Eleanor Vance & Arthur Pendelton). <span className="text-amber-600 font-bold bg-amber-50 px-1 rounded">3 assessments are unassigned</span> for tomorrow.
            </p>
          </div>
          <button
