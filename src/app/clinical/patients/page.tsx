...
export default function PatientsPage() {
  ...
  const filteredPatients = useMemo(() => {
    return allPatientsList.filter((patient: any) => {
      // Search match
  ...
  const stats = useMemo(() => {
    ...
    const inactiveCount = allPatientsList.filter((p: any) => p.status === "Care Completed").length;
  ...
  return (
    <div className="w-full space-y-4">
      ...
      {filteredPatients.length === 0 && (
        <div className="bg-white rounded-2xl p-10 text-center border border-slate-200 shadow-[0_4px_20px_rgba(0,0,0,0.02)] my-6">
          ...
          {viewMode === "list" ? (
                  ...
                  <tbody className="divide-y divide-slate-100 text-slate-700">
                    {filteredPatients.map((patient: any) => (
                      <tr
                        ...
                        onClick={() => router.push(`/clinical/patients/${patient.id}`)}
                      >
                        <td className="px-4 sm:px-6 py-3">
                          <div className="flex items-center gap-3">
                            ...
                            />
                            <div>
                              <div className="font-medium text-slate-900 group-hover:text-brand-teal transition-colors flex items-center gap-2">
                                {patient.name}
                              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
              {filteredPatients.map((patient: any, index: number) => (
                ...
                >
                  <Link href={`/clinical/patients/${patient.id}`} className="block h-full group">
                    <Card className="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200/90 shadow-[0_4px_24px_rgba(0,0,0,0.03)] hover:-translate-y-1 hover:shadow-[0_12px_36px_rgba(0,0,0,0.08)] hover:border-brand-teal/50 transition-all duration-200 relative overflow-hidden flex flex-col justify-between h-full">
                      <div>
                        ...
                        <div className="flex items-start justify-between gap-3 mb-4">
                          <div className="flex items-center gap-3.5 min-w-0">
                            ...
                            <div className="min-w-0">
                              <h3 className="font-semibold text-slate-900 group-hover:text-brand-teal transition-colors truncate text-base">
                                {patient.name}
                              </h3>
                        ...
                        <div className="space-y-2.5 mb-5 text-xs">
                          ...
                          <div className="flex justify-between items-center bg-slate-50/70 px-3 py-1.5 rounded-xl border border-slate-200">
                            <span className="text-slate-500 font-medium text-xs">Risk Level</span>
                            <Badge
                              variant={
                                patient.riskLevel === "Low"
                                  ? "success"
                          ...
                          <div className="flex justify-between items-start bg-slate-50/70 px-3 py-2 rounded-xl border border-slate-200 gap-2">
                            <span className="text-slate-500 font-medium shrink-0 text-xs">Diagnosis</span>
                            <span
                              className="font-semibold text-slate-800 text-right leading-tight break-words min-w-0 text-xs"
                              title={patient.primaryDiagnosis}
                            >
                              {patient.primaryDiagnosis}
                            </span>
                      ...
                      {/* Card Footer Link */}
                      <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-brand-teal text-xs sm:text-sm font-semibold tracking-tight">
