              ...
              return (
                ...
                >
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-3.5 sm:gap-4 items-center">
                    ...
                    {/* Instant Call-Off Replacement Cascade */}
                    <div className="lg:col-span-7 border-t lg:border-t-0 lg:border-l border-slate-200/80 pt-3 lg:pt-0 lg:pl-4">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-[10px] sm:text-[11px] font-bold text-slate-600 uppercase tracking-wider flex items-center gap-1">
                          <Sparkles className="w-3 h-3 text-brand-teal" />
                          Instant SMS Cascade
                        </span>
                        <span className="text-[10px] sm:text-[11px] text-slate-500 font-medium bg-slate-100 px-2 py-0.5 rounded-full animate-pulse">
                          Awaiting replies...
                        </span>
                      </div>
                      
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {co.suggestedReplacements.map((rep, idx) => (
                          <div
                            key={rep.id}
                            className={clsx(
                              "border rounded-xl p-2.5 flex items-center justify-between transition-all relative overflow-hidden",
                              idx === 0 ? "bg-brand-teal/5 border-brand-teal/30" : "bg-slate-50 border-slate-200 hover:bg-white hover:border-brand-teal/20"
                            )}
                          >
                            ...
                            <div className="shrink-0 flex items-center">
                              {idx === 0 ? (
                                <button
                                  ...
                                  className="px-2.5 py-1.5 rounded-lg bg-brand-teal hover:bg-teal-600 text-white text-[10px] font-bold shadow-xs active:scale-95 transition-all flex items-center gap-1"
                                >
                                ...
                                <button
                                  onClick={() => handleReassign(co.id, rep.id, rep.name)}
                                  className="flex items-center gap-1 text-[10px] font-medium text-slate-600 hover:text-brand-teal bg-white hover:bg-teal-50 px-2 py-1.5 rounded-lg border border-slate-200 transition-colors cursor-pointer"
                                >
