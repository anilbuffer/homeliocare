...
export function CarePlanTab({ patient }: { patient: Patient }) {
  ...
  return (
    <div className="space-y-6">
      {/* Approval Banner */}
      <div className="bg-emerald-50 border border-emerald-200 text-emerald-800 text-sm font-medium px-4 py-3 rounded-lg flex items-center">
      ...
      {/* Problems & Goals */}
      <div>
        <h3 className="text-sm lg:text-lg font-semibold text-slate-700 mb-3">Problems & goals</h3>
        <Card className="divide-y divide-slate-100 bg-white backdrop-blur-xl rounded-2xl p-0 border border-slate-200 shadow-[0_6px_32px_rgba(0,0,0,0.06)] hover:shadow-[0_10px_40px_rgba(0,0,0,0.1)] hover:border-brand-teal/60 transition-all duration-300 relative overflow-hidden">
              ...
              <div key={problem.id} className="flex flex-col">
                ...
                {isExpanded && (
                  <div className="px-4 pb-4 pt-1 bg-slate-50 border-t border-slate-100 text-sm text-slate-600">
                    <p className="mb-2"><strong>Goal:</strong> Achieve target metrics by {problem.targetDate}.</p>
                    <p><strong>Interventions:</strong> Monitor progress and adjust treatment as necessary. Ensure patient compliance with prescribed care activities.</p>
                  </div>
                )}
              </div>
            );
            ...
            <table className="w-full text-left text-sm text-slate-600">
              <thead className="bg-slate-50 text-xs text-slate-500 font-semibold uppercase tracking-wider">
                <tr>
                  ...
                  <th className="px-4 py-3 border-b border-slate-100">Assigned</th>
                  <th className="px-4 py-3 border-b border-slate-100">Last Completed</th>
                  <th className="px-4 py-3 border-b border-slate-100">Next Due</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {carePlan.tasks.map((task) => (
                  <tr key={task.id} className="hover:bg-slate-50/50">
                    <td className="px-4 py-3 font-medium text-slate-800">{task.task}</td>
                    <td className="px-4 py-3">{task.frequency}</td>
                    <td className="px-4 py-3">{task.assigned}</td>
                    <td className="px-4 py-3">{task.lastCompleted}</td>
                    <td className="px-4 py-3">{task.nextDue}</td>
                  </tr>
                ))}
              </tbody>
            </table>
