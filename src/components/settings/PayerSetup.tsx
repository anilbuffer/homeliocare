    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          {payers.map((payer) => (
            <Card key={payer.id} className="p-4 flex items-center justify-between group hover:border-brand-teal/30 transition-colors">
              <div className="flex items-center gap-4">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${payer.type === "Government" ? "bg-emerald-100 text-emerald-800" :
                  payer.type === "Direct" ? "bg-purple-100 text-purple-800" :
              ...
              <div className="flex flex-col items-end gap-2">
                <span className={`px-2.5 py-1 text-[10px] uppercase tracking-wider font-semibold rounded-full border ${payer.status === "Active" ? "bg-emerald-100 text-emerald-800 border-emerald-200" : "bg-amber-100 text-amber-800 border-amber-200"
                  }`}>
                  {payer.status}
                </span>
                <button className="text-xs font-medium text-brand-teal hover:text-teal-900 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <div className="space-y-6">
          <div className="bg-slate-800 text-white p-6 rounded-2xl shadow-lg relative overflow-hidden">
            ...
            <p className="text-xs text-slate-300 mb-6">Your agency is currently connected to 2 active clearinghouses.</p>
