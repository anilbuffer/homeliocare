          ...
          <tbody>
            {reports.map((report) => (
              <tr key={report.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors last:border-0">
                ...
                <td className="px-4 py-3">{report.schedule}</td>
                <td className="px-4 py-3 text-xs max-w-xs truncate" title={report.recipients}>{report.recipients}</td>
                <td className="px-4 py-3">{report.format}</td>
                <td className="px-4 py-3">{new Date(report.nextRunDate).toLocaleDateString()}</td>
                <td className="px-4 py-3">
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${report.active ? "bg-emerald-50 text-emerald-600 border border-emerald-200" : "bg-slate-100 text-slate-500 border border-slate-200"
                    }`}>
