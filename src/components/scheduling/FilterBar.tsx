        ...
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1 md:pb-0 hide-scrollbar">
          {SHIFT_FILTERS.map((filter) => (
            <button
              ...
              className={clsx(
                "px-3.5 py-2 min-h-[36px] rounded-xl text-xs font-bold transition-all whitespace-nowrap border shrink-0 flex items-center justify-center",
                activeFilter === filter
                  ? "bg-brand-teal text-white border-brand-teal shadow-[0_8px_30px_rgb(0,0,0,0.04)]"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 pt-2 border-t border-slate-100">
        <div className="relative">
          <select
            ...
            className="w-full appearance-none flex items-center gap-2 pl-3.5 pr-8 py-2.5 min-h-[44px] bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-brand-teal/20"
          >
