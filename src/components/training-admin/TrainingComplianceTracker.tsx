              ...
              <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {req.status === "compliant" && <span className="bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full text-[10px] font-medium">Satisfied</span>}
                    {req.status === "review-needed" && <span className="bg-red-100 text-red-700 px-2 py-0.5 rounded-full text-[10px] font-medium">Gap / Action Needed</span>}
                    {req.status === "in-progress" && <span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full text-[10px] font-medium">Building Course</span>}
                  </div>

                  <p className="text-[10px] text-slate-600 mb-4">{req.regulatoryBasis}</p>

                  <div className="flex items-center gap-4 text-xs font-medium text-slate-500">
                    <span className="flex items-center gap-1"><Calendar className="w-3 h-3" /> {req.frequency}</span>
                ...
                <div className="lg:w-1/3 bg-slate-100 rounded-xl px-3 py-2 border border-slate-200">
                  ...
                  <div className="space-y-2">
                    {req.satisfyingCourses.map(course => (
                      <div key={course.id} className="flex justify-between items-center text-sm bg-white px-2 py-1 border border-slate-100 rounded-lg shadow-[0_6px_32px_rgba(0,0,0,0.06)]">
