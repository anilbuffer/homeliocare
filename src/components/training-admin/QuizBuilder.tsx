...
export function QuizBuilder() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-20">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 p-5 bg-white backdrop-blur-xl rounded-2xl border border-slate-200 shadow-[0_6px_32px_rgba(0,0,0,0.06)] hover:-translate-y-1 hover:shadow-[0_10px_40px_rgba(0,0,0,0.1)] hover:border-brand-teal/60 transition-all duration-300 relative overflow-hidden sticky top-4 z-20">
        <div>
          <input
            type="text"
            value={quizTitle}
            onChange={(e) => setQuizTitle(e.target.value)}
            className="text-2xl font-bold text-slate-800 bg-transparent border-none focus:ring-0 p-0 hover:bg-slate-50 rounded"
          />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-white backdrop-blur-xl rounded-2xl border border-slate-200 shadow-[0_6px_32px_rgba(0,0,0,0.06)] hover:-translate-y-1 hover:shadow-[0_10px_40px_rgba(0,0,0,0.1)] hover:border-brand-teal/60 transition-all duration-300 relative overflow-hidden p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-slate-800">Question Bank ({questions.length})</h2>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => handleAddQuestion('multiple-choice')}
                  className="px-3 py-1.5 text-sm font-medium text-brand-teal bg-brand-teal/10 rounded-lg hover:bg-brand-teal/20 transition-colors flex items-center gap-1"
                >
              ...
              {questions.map((q, index) => {
                ...
                return (
                  <div key={q.id} className={`border ${isExpanded ? 'border-brand-teal shadow-sm' : 'border-slate-200 hover:border-slate-300'} rounded-xl bg-white transition-all overflow-hidden`}>
                    ...
                    {/* Header */}
                    <div
                      className={`flex items-start gap-3 p-4 cursor-pointer ${isExpanded ? 'bg-slate-50 border-b border-slate-100' : ''}`}
                      onClick={() => setExpandedQuestion(isExpanded ? null : q.id)}
                    >
                      <div className="mt-1 cursor-grab text-slate-400 hover:text-slate-600" onClick={(e) => e.stopPropagation()}>
                        <GripVertical className="w-4 h-4" />
                      </div>
                      <div className="w-6 h-6 rounded bg-slate-200 text-slate-600 flex items-center justify-center text-xs font-bold shrink-0">
                        {index + 1}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="font-semibold text-slate-800 line-clamp-2">{q.text || "New Question..."}</div>
          ...
          <div className="bg-white backdrop-blur-xl rounded-xl border border-slate-200 shadow-[0_6px_32px_rgba(0,0,0,0.06)] hover:-translate-y-1 hover:shadow-[0_10px_40px_rgba(0,0,0,0.1)] hover:border-brand-teal/60 transition-all duration-300 relative overflow-hidden p-4">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-2">Learners scoring below this threshold will be required to retake the quiz.</p>
              </div>
