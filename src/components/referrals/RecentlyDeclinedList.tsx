    ...
    <div className="bg-white backdrop-blur-xl rounded-2xl p-4 border border-slate-200 shadow-[0_6px_32px_rgba(0,0,0,0.06)] hover:-translate-y-1 hover:shadow-[0_10px_40px_rgba(0,0,0,0.1)] hover:border-brand-teal/60 transition-all duration-300 relative overflow-hidden flex-1 flex flex-col max-h-80">
      ...
      {isExpanded && (
        <div className="mt-3 space-y-2 bg-white flex-1 overflow-y-auto pr-1 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-slate-200 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-slate-300">
          {declined.map(item => (
            <div key={item.id} className="px-3 py-2 bg-slate border border-slate-100 hover:bg-slate-100 rounded-lg transition-colors flex justify-between items-center">
              <div>
