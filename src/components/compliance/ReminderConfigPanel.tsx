...
export function ReminderConfigPanel({ logs }: ReminderConfigPanelProps) {
  ...
  return (
    <div className="bg-white backdrop-blur-xl rounded-2xl border border-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden flex flex-col h-full">
        ...
        {logs.map((log) => (
          <div key={log.id} className="flex items-start gap-3 bg-white p-4 rounded-xl border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)]  hover:shadow-sm hover:-translate-y-0.5 transition-all duration-200">
            ...
            <div className="flex flex-col items-end gap-1.5">
              <span className={`text-[10px] uppercase font-bold tracking-wide px-2 py-0.5 rounded shadow-[0_8px_30px_rgb(0,0,0,0.04)] border ${log.status === "Sent" ? "bg-slate-50 border-slate-200 text-slate-500" :
