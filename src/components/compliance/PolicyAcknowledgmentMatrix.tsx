  ...
  return (
    <div className="bg-white backdrop-blur-xl rounded-2xl border border-slate-200 shadow-[0_6px_32px_rgba(0,0,0,0.06)] hover:-translate-y-1 hover:shadow-[0_10px_40px_rgba(0,0,0,0.1)] hover:border-brand-teal/60 transition-all duration-300 relative overflow-hidden flex flex-col">
      <div className="p-4 border-b border-slate-200 bg-white/50">
        ...
        <p className="text-xs text-slate-500 mt-1">Track staff signatures on critical agency policies.</p>
      </div>
      ...
      <div className="overflow-x-auto [&::-webkit-scrollbar]:h-2 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-slate-200 [&::-webkit-scrollbar-thumb]:rounded-full">
        <table className="w-full text-left border-collapse min-w-[600px]">
          <thead className="bg-slate-50/95 backdrop-blur-sm text-[11px] uppercase tracking-wider text-slate-500 font-bold border-b border-slate-200">
            <tr>
              <th className="px-4 py-3 rounded-tl-lg">Policy Name</th>
              <th className="px-4 py-3">Applicable Roles</th>
              {uniqueCaregivers.map(cg => (
                <th key={cg} className="px-4 py-3 text-center truncate max-w-[100px]" title={cg}>
                  {cg.split(" ")[0]}
                </th>
              ))}
            </tr>
            ...
            {policies.map(policy => (
              <tr key={policy.id} className="hover:bg-slate-50/80 transition-colors duration-200">
                ...
                {uniqueCaregivers.map(cgName => {
                  ...
                  return (
                    <td key={cgName} className="px-4 py-3 text-center">
                      {isAcknowledged ? (
