...
export function VerificationQueue({ items: initialItems }: VerificationQueueProps) {
  ...
  const handleApprove = (id: string, name: string) => {
    ...
    toast.success(`Approved document for ${name}`);
  };

  const handleReject = (id: string, name: string) => {
    setItems(prev => prev.filter(item => item.id !== id));
    toast.error(`Rejected document for ${name}`);
  };
  ...
  return (
    <div className="bg-white backdrop-blur-xl rounded-2xl p-4 border border-slate-200 shadow-[0_6px_32px_rgba(0,0,0,0.06)] hover:-translate-y-1 hover:shadow-[0_10px_40px_rgba(0,0,0,0.1)] hover:border-brand-teal/60 transition-all duration-300 relative overflow-hidden flex flex-col h-auto lg:h-full">
      <div className="pb-4 border-b border-slate-200 flex items-center justify-between bg-white/50">
        ...
        <span className="bg-brand-teal/20 text-brand-teal text-xs font-medium px-2 py-1 rounded-full">
          ...
          {items.length === 0 ? (
            ...
            items.map((item) => (
              <motion.div
                ...
                className="px-4 py-4 bg-slate-50 hover:bg-slate-100 transition-colors duration-200 group"
              >
                <div className="flex items-start gap-4">
                  <div className="relative shrink-0">
                    <div className="w-12 h-12 bg-slate-50 rounded-xl border border-slate-200 flex items-center justify-center group-hover:border-brand-teal/30 transition-colors">
